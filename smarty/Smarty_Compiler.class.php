...
class Smarty_Compiler extends Smarty {
    ...
    function _parse_modifiers(&$output, $modifier_string)
    {
        ...
        for ($_i = 0, $_for_max = count($_modifiers); $_i < $_for_max; $_i++) {
			...
			// dmittner hacks to provide $smarty in modifiers as first argument to select modifiers
			$modList = array("tr","str");
            if ($_map_array) {
                if ( in_array($_modifier_name,$modList) ){
                    $output = "((is_array(\$_tmp=$output)) ? \$this->_run_mod_handler('$_modifier_name', true, \$this,\$_tmp$_modifier_args) : " . $this->_compile_plugin_call('modifier', $_modifier_name) . "(\$this,\$_tmp$_modifier_args))";
                } else {
                    $output = "((is_array(\$_tmp=$output)) ? \$this->_run_mod_handler('$_modifier_name', true, \$_tmp$_modifier_args) : " . $this->_compile_plugin_call('modifier', $_modifier_name) . "(\$_tmp$_modifier_args))";
                }
            } else {
                if ( in_array($_modifier_name,$modList) ){
                    $output = $this->_compile_plugin_call('modifier', $_modifier_name)."(\$this,$output$_modifier_args)";
                } else {
